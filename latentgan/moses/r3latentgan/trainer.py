...
class R3LatentGANTrainer(MosesTrainer):
    ...
    def _train_epoch(self, model, tqdm_data, optimizer_disc=None, optimizer_gen=None):
        ...
        for i, real_mols in enumerate(tqdm_data):
            ...
            if optimizer_disc is not None:
                ...
                # Generate fresh fake samples for discriminator training
                ...
                # Discriminator loss
                adv_loss = torch.nn.functional.softplus(fake_validity - real_validity)
                d_loss = torch.mean(adv_loss) + (self.config.gp / 2) * gradient_penalty
                ...
                if i % self.config.n_critic == 0:
                    ...
                    # Generator loss
                    adv_loss = torch.nn.functional.softplus(real_validity - fake_validity)  # Simplified version
                    g_loss = torch.mean(adv_loss)
