...
class Solver(object):
    ...
    def __init__(self, config, log=None, wandb_=False):
    ...
    def reward(self, mols):
        ...
        for metric in metrics_to_use:
            ...
            reward_score = reward_score * metric_calculators[metric](mols)
    ...
    def train_and_validate(self):
        ...
        # Start training.
        # early stop for optuna
        best_quality, i_best = 0.0, 0
        ...
        if self.mode == "train":
            ...
            for i in range(start_epoch, self.num_epochs):
                ...
                if (i + 1) % self.eval_freq == 0:
                    ...
                    # early stop for optuna
                    #quality_on_validation = self.quality_scores[-1]
                    #if quality_on_validation >= best_quality:
                    #    best_quality = quality_on_validation
                    #    i_best = i
                    #elif (i - i_best) >= 25 and (best_quality > quality_on_validation):
    ...
    def train_epoch(self, epoch):
        ...
        for a_step in range(self.num_steps):
            ...
            #r1 - real 
            x_int0 = a_tensor.detach().requires_grad_(True)
            x_int1 = x_tensor.detach().requires_grad_(True)
            
            grad0, grad1 = self.D(x_int0, None, x_int1)
            r1_penalty = self.gradient_penalty(grad0, x_int0) + self.gradient_penalty(
                grad1, x_int1
            )
            
            #r2 - fake
            x_int0 = edges_hat.detach().requires_grad_(True)
            x_int1 = nodes_hat.detach().requires_grad_(True)
            
            grad0, grad1 = self.D(x_int0, None, x_int1)
            r2_penalty = self.gradient_penalty(grad0, x_int0) + self.gradient_penalty(
                grad1, x_int1
            )

            relative_logits = logits_real - logits_fake
            d_loss = torch.nn.functional.softplus(-relative_logits)
            loss_D = torch.mean(d_loss + self.la_gp / 2 * (r1_penalty + r2_penalty))

            losses["l_r1"].append(torch.mean(r1_penalty).item())
            losses["l_r2"].append(torch.mean(r2_penalty).item())
            ...
            # Detach these tensors to prevent them from being part of the generator's graph
            #edges_hat_detached = edges_hat.detach()
            #nodes_hat_detached = nodes_hat.detach()
            ...
            loss_G_adv = torch.mean(torch.nn.functional.softplus(relative_logits))
            ...
            if la_curr < 1.0:
                ...
                # Use the detached versions for value network computation
                value_logit_fake, _ = self.V(edges_hat.detach(), None, nodes_hat.detach(), torch.sigmoid)
            
                reward_r = torch.from_numpy(self.reward(mols)).to(self.device)
                reward_f = self.get_reward(nodes_hat.detach(), edges_hat.detach(), self.post_method)
            ...
            if cur_step % self.n_critic == 0:
                ...
                if self.la < 1.0:
                    self.v_optimizer.zero_grad()
                    # Don't need to retain graph anymore since we're using detached tensors
                    loss_V.backward()
    ...
    def validate_epoch(self, epoch, val_n=None):
        ...
        self.quality_scores.append(metrics_acc['quality'][0])
        # average & report
        # report = ", ".join(
        ...
        if self.wandb:
            ...
            for tag, vals in metrics_acc.items():
                log_dict[tag] = np.mean(vals)
